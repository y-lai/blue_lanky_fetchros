  ...
  const sensor_msgs::Image::ConstPtr& msg)
{
  ...
  // Convert to 3d
  cv::Mat points3d;
  cv::rgbd::depthTo3d(cv_ptr->image, K_, points3d);
  ...
  if (normals_estimator_.empty())
  {
    ...
    //                                          cv_ptr->image.depth(),
    //                                          K_);
    normals_estimator_ = new cv::rgbd::RgbdNormals(cv_ptr->image.rows,cv_ptr->image.cols,cv_ptr->image.depth(),K_);
  }
  cv::Mat normals;
  ...
  if (plane_estimator_.empty())
  {
    ...
    // plane_estimator_ = cv::rgbd::RgbdPlane("RGBD.RgbdPlane");
    plane_estimator_ = new cv::rgbd::RgbdPlane();
